...
stopMiddleware() {
    ...
    ssh -o StrictHostKeyChecking=no junkerp@${ip} "bash -c 'screen -X -S ${designator} quit; ls; cnt=0; while [[ ! -f ~/asl/logs/done.info && ${cnt} -lt 50 ]]; do cnt=$((cnt + 1)); sleep 0.1; done; rm ~/asl/logs/done.info;'"
}
