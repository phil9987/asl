...
runMemtierClientLocalMultiget() {
    ...
    basecmd="memtier_benchmark --server=$1 --port=$2 --clients=$3 --test-time=${TESTTIME} --ratio=1:$4 --multi-key-get=$4 --protocol=memcache_text --run-count=1 --threads=$6 --key-maximum=10000 --data-size=4096 --client-stats=../logs/${logname}clientstats --json-out-file=../logs/${logname}.json"
    if [[ $# -eq 6 ]]; then
